...
CREDENZIALI = {
    ...
    "TLA2027":  {"code": "P01", "ruolo": "Team Leader",          "ordine": ["T1","T2","T3"]},
...
LEADERSHIP_ROLES = ["Team Leader", "Secondo Team Leader", "Civile Leader"]
...
def infer_roles(df):
    ...
    # Priorità 3: fallback posizionale coerente con il team ampliato a 11 agenti
    struttura = (
        ["Team Leader", "Secondo Team Leader", "Civile Leader"] +
        ["Analista Senior"] * 3 +
        ["Analista Junior"] * 3 +
        ["Analista Civile"] * 2
    )
    ...
    # Deve esistere un Team Leader primario; gli altri coordinatori restano distinti.
    if not (df["role"] == "Team Leader").any() and not df.empty:
...
def build_network(team, condition):
    """Costruisce una rete a 11 agenti con leadership plurale ma non equivalente."""
    H = COND_PARAMS[condition]["H"]
    G = nx.Graph()
    for _, row in team.iterrows():
        G.add_node(row["participant_code"], role=row["role"])

    primary = team[team["role"] == "Team Leader"]["participant_code"].tolist()
    primary = primary[0] if primary else team.iloc[0]["participant_code"]
    deputy = team[team["role"] == "Secondo Team Leader"]["participant_code"].tolist()
    civil_leaders = team[team["role"] == "Civile Leader"]["participant_code"].tolist()
    coordinators = [primary] + deputy + civil_leaders
    ...
    # Il leader primario mantiene la massima centralità; gli altri coordinatori
    # fungono da nodi intermedi, evitando di ridurre la rete a una stella pura.
    for n in team["participant_code"]:
        if n != primary:
            G.add_edge(primary, n, weight=0.28 + 0.58 * H)

    for c in deputy + civil_leaders:
        for n in seniors + juniors + civils:
            if c != n:
                G.add_edge(c, n, weight=0.20 + 0.42 * H)

    # Collegamento tra le figure di coordinamento
    for i in range(len(coordinators)):
        for j in range(i + 1, len(coordinators)):
            G.add_edge(coordinators[i], coordinators[j], weight=0.55 + 0.35 * H)
    ...
    for sr in seniors:
        for jr in juniors:
            G.add_edge(sr, jr, weight=0.55 * (1 - H) + 0.08)
        for cv in civils:
            G.add_edge(sr, cv, weight=0.65 * (1 - H) + 0.08)
    for i in range(len(juniors)):
        for j in range(i + 1, len(juniors)):
            G.add_edge(juniors[i], juniors[j], weight=0.35 * (1 - H) + 0.04)
    for cv in civils:
        for jr in juniors:
            G.add_edge(cv, jr, weight=0.25 * (1 - H) + 0.04)
    return G
...
def simulate_condition(team, condition, seed):
    ...
    G_i    = np.array([compute_G(r, condition)
                       for _, r in team.iterrows()])
    roles = team["role"].to_numpy()
    leadership_idx = np.array([i for i, role in enumerate(roles)
                               if role in LEADERSHIP_ROLES], dtype=int)
    if leadership_idx.size == 0:
        leadership_idx = np.array([0], dtype=int)
    leadership_weights = team.iloc[leadership_idx]["role_influence"].astype(float).to_numpy()
    history = []

    for step in range(steps+1):
        leader_b = float(np.average(beliefs[leadership_idx],
        ...
        leadership_initial = float(np.average(initial[leadership_idx],
                                              weights=leadership_weights))
        dist_l0   = np.mean(np.abs(initial - leadership_initial)) + 1e-6
...
def perturb_team(team, seed, noise=0.08):
    ...
    La struttura dei ruoli rimane invariata: 1 Team Leader, 1 Secondo Team
    Leader, 1 Civile Leader, 3 Senior, 3 Junior e 2 Civili. Vengono perturbati
    solo i parametri cognitivi
...
def plot_network(team):
    ...
    colori_ruolo = {
        "Team Leader":         "#E53935",
        "Secondo Team Leader": "#8E24AA",
        "Civile Leader":       "#6D4C41",
