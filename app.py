...
COND_LABEL = {
    "T1": "Configurazione α — Tempo ampio, contesto ordinario",
    "T2": "Configurazione β — Tempo ridotto, contesto instabile",
    "T3": "Configurazione γ — Emergenza operativa, decisione immediata",
}

COND_CONTESTO = {
    "T1": """Il team di analisi opera in condizioni ordinarie.
Non è stata dichiarata alcuna emergenza.
Il Team Leader ha convocato una sessione di lavoro standard.
Il tempo disponibile per la valutazione individuale è adeguato.
Non vi sono vincoli di trasmissione imminenti verso l'autorità superiore.""",

    "T2": """Il CASA è stato convocato in sessione straordinaria.
Il Team Leader deve trasmettere una sintesi analitica all'autorità superiore entro **due ore**.
Il tempo disponibile per la valutazione individuale è ridotto.
È richiesta una risposta analitica rapida ma motivata.
La situazione operativa mostra segnali di crescente instabilità.""",

    "T3": """🔴 **EMERGENZA OPERATIVA DICHIARATA.**
Il Direttore ha richiesto una valutazione immediata.
Il Team Leader deve trasmettere la sintesi decisionale entro **trenta minuti**.
Non è possibile attendere ulteriori approfondimenti.
La valutazione individuale deve essere formulata adesso.""",
}
...
# Varia: intensità degli eventi, urgenza dichiarata, volume informativo
...
Il quadro informativo delle ultime settimane segnala un incremento di indicatori relativi a possibili azioni ostili contro infrastrutture critiche nazionali. Nessun evento specifico è ancora occorso. La valutazione è richiesta in via precauzionale.
...
Nelle ultime ore si è verificato un evento che modifica il peso degli indicatori precedentemente valutati. Il quadro richiede una rivalutazione urgente sulla base degli elementi aggiornati.
...
Il quadro informativo ha subito una grave deteriorazione nelle ultime ore. Si registra una seconda azione ostile con potenziale coinvolgimento di attori statuali. La decisione non può essere rinviata.
...
def plot_descriptive(df):
    ...
    plt.xticks(x, ["T1","T2","T3"]); plt.ylim(0,1.05)
...
def infer_roles(df):
    """
    Ruoli assegnati dal ricercatore prima della somministrazione.
    Nessuna inferenza algoritmica — lookup diretto da RUOLI_FISSI.
    Le variabili di profilo (experience, coordination, ai_critical_skill)
    rimangono come variabili di controllo della coerenza.
    """
    df = df.copy()
    # Ruolo da colonna salvata nel DB (assegnato al momento dell'accesso PIN)
    if "ruolo_assegnato" in df.columns:
        df["role"] = df["ruolo_assegnato"]
    else:
        # Fallback: lookup da RUOLI_FISSI per codice partecipante
        df["role"] = df["participant_code"].map(RUOLI_FISSI)
    df["role"] = df["role"].fillna("Analista Junior")
    df["role_influence"] = df["role"].map(ROLE_INFLUENCE)
    ...
    df["leadership_index"] = 0.35*df["E"]+0.30*df["C"]+0.20*df["S"]+0.15*df["A"]
    df["seniority_index"]  = 0.50*df["E"]+0.20*df["A"]+0.20*df["S"]+0.10*df["C"]
...
def run_montecarlo(team, n=N_MC):
    res = []
    for run in range(n):
        for cond in ["T1","T2","T3"]:
            sim = simulate_condition(team, cond, BASE_SEED+run+1000)
            sim["run"] = run; res.append(sim)
    ...
    finals = results.sort_values("step").groupby(
        ["run","condition"], as_index=False).tail(1)
    ...
    for col in ["mediation","delegation","hierarchy","consensus"]:
        summary[f"{col}_ci95"] = 1.96*summary[f"{col}_sd"]/np.sqrt(n)
...
def plot_network(team):
    G = build_network(team, "T1")
    pos = nx.spring_layout(G, seed=42, k=0.8)
    plt.figure(figsize=(11,8))
    sizes = {"Team Leader":1600,"Analista Senior":1150,
             "Analista Junior":950,"Analista Civile":1050}
    weights = [G[u][v]["weight"] for u,v in G.edges()]
    for role in sizes:
        nodes = [n for n,d in G.nodes(data=True) if d["role"]==role]
        nx.draw_networkx_nodes(G, pos, nodelist=nodes,
                               node_size=[sizes[role]]*len(nodes),
                               alpha=0.9, label=role)
    nx.draw_networkx_edges(G, pos,
                           width=[1+4*w for w in weights], alpha=0.35)
    nx.draw_networkx_labels(G, pos,
        labels={n:f"{n}\n{G.nodes[n]['role']}" for n in G.nodes()},
        font_size=8)
    plt.title("Rete ABM del Sistema 3", fontsize=15)
    plt.axis("off"); plt.legend(loc="lower left")
    plt.tight_layout(); plt.savefig(str(FIG_NET), dpi=200); plt.close()
...
def plot_abm_single(abm):
    ...
    for metric, label in [
        ...
        plt.plot(["T1","T2","T3"], finals.values,
...
if admin != ADMIN_PWD:
    ...
    st.caption(
        ...
        f"Ruolo: {ruolo_assegnato} — "
        f"Ordine scenari: {' → '.join(ordine)}")
...
""")
    ...
    for T in ordine:
        ...
        if not st.session_state[f"{T}_confirmed"]:
            ...
            if st.button(
                    f"✅ Conferma e passa alla Sezione II — {T}",
        ...
        motivo = st.selectbox(
            f"{T} — Se hai modificato la tua valutazione, qual è il motivo principale?",
            motivo_options,
            key=f"{T}_motivo")
        responses[f"{T}_motivo_aggiornamento"] = motivo

        st.subheader("Sezione IV — Valutazione del contesto")
        responses[f"{T}_gravity"]           = st.slider(f"{T} — Gravità percepita del contesto", 1, 7, 4, key=f"{T}_grav")
        responses[f"{T}_uncertainty"]       = st.slider(f"{T} — Incertezza percepita dello scenario", 1, 7, 4, key=f"{T}_unc")
        responses[f"{T}_strategic"]         = st.slider(f"{T} — Impatto strategico potenziale", 1, 7, 4, key=f"{T}_str")
        responses[f"{T}_trust_ai"]          = st.slider(f"{T} — Fiducia attribuita al Sistema AI", 1, 7, 4, key=f"{T}_tr")
        responses[f"{T}_confidence"]        = st.slider(f"{T} — Sicurezza della tua decisione finale", 1, 7, 4, key=f"{T}_conf")
        responses[f"{T}_leader_acceptance"] = st.slider(f"{T} — Disponibilità ad accettare la sintesi del Team Leader", 1, 7, 4, key=f"{T}_la")
        responses[f"{T}_need_group"]        = st.slider(f"{T} — Necessità percepita di confronto con gli altri analisti", 1, 7, 4, key=f"{T}_ng")
...
c3.metric("AI costante", f"{AI_COSTANTE}%")
c4.metric("Monte Carlo", N_MC)
...
with tabs[6]:
    for f in [CSV_RESPONSES,CSV_DESC,CSV_TEAM,CSV_ABM,
              CSV_MC,CSV_MC_SUM,FIG_DESC,FIG_NET,
